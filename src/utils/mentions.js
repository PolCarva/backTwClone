...
async function mention(text, userId, userUsername, postId, type){
	...
	for (let i = 0; i < words.length; i++) {
		if(words[i][0] === '@'){
			...
			if(user && user.dataValues.id !== userId && !usersMentioned.includes(user.dataValues.id)){
				await notificationsApi.createNotification(mentionTitle(), mentionMessage(userUsername, type), user.dataValues.id, 'mention', null, null, postId);
